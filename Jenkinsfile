pipeline {
    agent {
        docker {
            image 'maven:3.3.9'
            label 'docker'
            args '-v /usr/bin/docker:/usr/bin/docker -v /var/run/docker.sock:/var/run/docker.sock -v ${HOME}/.m2:${WORKSPACE}/.m2'
        }
    }
    ...
    stages {
        ...
        stage ('Deploy') {
            steps {
                sh 'env'
                sh 'mvn deploy:deploy -gs ${WORKSPACE}/.m2/settings.xml -C -B -Pimage -DskipTests -DdockerRepository=${DOCKER_REPOSITORY}'
            }
        }
     }
}
